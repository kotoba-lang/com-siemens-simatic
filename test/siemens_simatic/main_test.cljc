(ns siemens_simatic.main-test
  "Contract + behavioral test for the siemens_simatic-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "siemens_simatic-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'siemens_simatic.main-test)]
