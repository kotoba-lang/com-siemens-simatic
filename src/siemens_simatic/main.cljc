(ns siemens_simatic.main
  "Kotodama WASM entrypoint for the Siemens_simatic clean-room actor (L4) — Clojure port.
  ...
  facts (`siemens_simatic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "siemens_simatic")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "siemenss_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "siemenss_con"
    :fields [:name :setpoint :processValue :mode]      :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float}    :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "siemenss_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"siemens_simatic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "siemens_simatic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :siemens_simatic-compat/ready)
